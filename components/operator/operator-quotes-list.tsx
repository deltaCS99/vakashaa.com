// components/operator/operator-quotes-list.tsx
"use client";

import { useState } from "react";
import { OperatorQuoteCard } from "./operator-quote-card";
...
interface OperatorQuotesListProps {
...
export function OperatorQuotesList({ quoteRequests }: OperatorQuotesListProps) {
    ...
    const pendingCount = statusCounts[QuoteStatus.Pending] || 0;
    ...
    return (
        <div className="space-y-6">
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {STATUS_FILTERS.map((filter) => {
                            ...
                            const isPending = filter.value === QuoteStatus.Pending;
                            const hasPendingQuotes = isPending && count > 0;
                            ...
                            return (
                                <Button
                                    ...
                                    className={`relative ${hasPendingQuotes && !isActive ? "animate-pulse" : ""
                                        }`}
                                >
                                    {filter.label}
                                    {count > 0 && (
                                        <span
                                            className={`ml-2 px-2 py-0.5 rounded-full text-xs ${isActive ? "bg-white/20" : "bg-primary/10"
                                                } ${hasPendingQuotes && !isActive ? "bg-yellow-100 text-yellow-800" : ""}`}
                                        >
                                            {count}
                                        </span>
                                    )}
            ...
            <div className="flex items-center justify-between">
                ...
                {pendingCount > 0 && selectedFilter === "all" && (
            ...
            {/* Quotes Grid - Single unified grid */}
            {filteredQuotes.length > 0 ? (
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {filteredQuotes.map((quote) => (
                        <OperatorQuoteCard key={quote.id} quote={quote} />
                    ))}
                </div>
            ) : (
                <Card className="p-8 text-center">
                    <p className="text-gray-600">No quotes found matching your filters.</p>
                    <Button
                        variant="link"
                        onClick={() => {
